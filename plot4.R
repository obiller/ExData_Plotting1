...
createPlot <- function(hpc_s) {
        ...
        # Graph (1,2)
        with(hpc_s, plot(DateTime, Voltage, xlab="datetime", ylab="Voltage", type="l") )
        # Graph (2,1)
        ...
        legend("topright", col=c("black", "red", "blue"), legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), seg.len=3, lty="solid", cex=1, bty="n")
        # Graph (2,2)
        with(hpc_s, plot(DateTime, Global_reactive_power, xlab="datetime", ylab="Global_reactive_power", type="l") )
        dev.off()
}

plot4Creator <- function() {
